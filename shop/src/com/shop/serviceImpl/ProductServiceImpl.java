...
//	查询热门商品 
	public List<Product> findHotProduct() throws Exception {
		 ...
		 criteria.andIsHotEqualTo(1);//添加xxx字段等于value条件
	...
	public List<Product> findNewProduct() throws Exception {
		 ProductExample example = new ProductExample();
		 ProductExample.Criteria criteria = example.createCriteria();
		 example.setOrderByClause("pdate DESC");//添加升序排列条件，DESC为降序
		 example.setBeginPage(0);
		 example.setEnd(8);		 
		 List<Product> list = productMapper.selectByExample(example);
		 /*for (Product product : list) {
...
//	根据id查找商品
	public Product productFindByPid1(int pid) throws Exception {
